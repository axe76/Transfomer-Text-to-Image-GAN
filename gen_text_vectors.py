# -*- coding: utf-8 -*-
"""
Created on Wed Jul 14 11:34:03 2021
...
def create_feature_vectors_for_single_comment2(word2vec_model, cleaned_comments, image_names):
    vectorized_list = []
    image_list = []
    ...
    for comments, image in zip(cleaned_comments, image_names):
        ...
        vectorized_list.append(np.asarray(result_array).astype('float32'))
        image_list.append(image)
...
def create_sentence_embeddings():
    ...
    i, c = create_feature_vectors_for_single_comment2(model, cleaned_captions, image_names)
    word_vector_dict = dict(zip(i, c))
    pickle.dump(word_vector_dict, open('word_vector_min_bird2' + ".p", "wb"))
    print('Done')
